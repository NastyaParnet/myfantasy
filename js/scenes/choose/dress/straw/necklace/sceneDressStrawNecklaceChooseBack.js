export default class sceneDressStrawNecklaceChooseBack extends Phaser.Scene {
    constructor(){
        super('sceneDressStrawNecklaceChooseBack')
    }
    preload(){
    ...
    create(){
        ...
        let girl = this.add.image(sceneWidth/2, sceneHeight/2, 'dress/straw/necklace/girl')
    ...
    update(){
        if(this.timer>0) {
            ...
            if(this.accessory=='beach') this.scene.start('sceneDressStrawNecklaceBeach');
            else this.scene.start('sceneDressStrawNecklaceSea');
        }
    }
}
